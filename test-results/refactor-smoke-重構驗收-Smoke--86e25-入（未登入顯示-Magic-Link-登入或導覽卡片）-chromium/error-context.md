...
- Name: refactor-smoke.spec.ts >> 重構驗收 Smoke 測試 >> 首頁能正常載入（未登入顯示 Magic Link 登入或導覽卡片）
- Location: tests\e2e\refactor-smoke.spec.ts:41:7
...
Locator: locator('text=PhamaCount')
Expected: visible
Timeout: 15000ms
...
Call log:
  ...
  - waiting for locator('text=PhamaCount')
...
```ts
  ...
  4   |  * 重構驗收 Smoke E2E 測試
  5   |  *
  6   |  * 目的：驗證 Phase 1~3 重構後，被拆分/共用的模組不會破壞頁面載入。
  7   |  * 本測試不依賴真實 Supabase 資料，只驗證：
  8   |  *   1. 頁面能正常載入（HTTP 200）
  9   |  *   2. 首頁關鍵元素與導覽連結正常
  10  |  *   3. 無未捕獲的 runtime 錯誤（console.error / pageerror）
  11  |  *   4. 被重構的頁面（/import, /manifests, /scan）能載入而不崩潰
  12  |  */
  13  | 
  14  | // 收集頁面錯誤
